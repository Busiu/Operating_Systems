...
//LICZNIK WYSLANYCH PROSB PRZEZ 1 PROCES
int numberOfRequests = 0;
//LICZNIK KTORY ODLICZA CZAS DO WYSLANIA KOLEJNEJ PROSBY
int counter = 5;
...
int main(int argc, char ** argv) {
	...
	for(int i = 0; i < N; i++){
		...
		if(childPID[i] == 0){
			...
			while(1){
				//JEZELI OTRZYMAMY POZWOLENIE
				if(isAllowed){
					break;
				}
				//JEZELI CZEKALISMY ZBYT DLUGO
				if(counter == 0){
				 	counter = 5;
					kill(getppid(), SIGUSR1);
					numberOfRequests++;
			...
			//WYPISYWANIE FUNKCJONALNOSCI
			if(func[4] == 't'){
				printf("Proces potomny o ID %i spal przez %i sekund, wyslal %i prosb\n", getpid(), sleepTime, numberOfRequests);
			}
	...
	//printf("Liczba otrzymanych sygnalow czasu rzeczywistego: %i\n", numberOfTerminations);
	//printf("Liczba faktycznie wylaczanych procesow: %i\n", N);
